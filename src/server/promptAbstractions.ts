...
export const getActionQueueFormat = (_currentDate: string): string[] => [
  '### Action queue — prioritized list of actions organized by confidence level (high/medium/low) and impact.',
  '',
  '**HIGH CONFIDENCE (>80%) — Execute immediately**:',
  'Make actual MCP tool calls for each high-confidence action.',
  'Example: [Tool call to reminders_tasks with action=update, id=ABC-123, completed=true]',
  'After execution, show: ✓ Marked "Task Name" as complete',
  '  - Tool: reminders_tasks, action: update, id: ABC-123, completed: true',
  '  - Rationale: Note indicates work is done',
  '',
  '**MEDIUM CONFIDENCE (60-80%) — Recommend with tool-ready format**:',
  '- [MEDIUM, 75%] Mark "Task X" as complete',
  '  - Tool: reminders_tasks, action: update, id: ABC-123, completed: true',
  '  - Rationale: Note indicates work is done',
  '',
  '**LOW CONFIDENCE (<60%) — Use AskUserQuestion tool**:',
  'For low-confidence actions, use AskUserQuestion to present options to the user.',
  'Do not list them as text descriptions - convert them into interactive questions.',
  'Example: [AskUserQuestion tool call with question and 2-4 options]',
  '',
